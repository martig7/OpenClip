...
function saveElectronMarkers(electronMarkers) {
  const trimmed =
    electronMarkers.length > MAX_MARKERS ? electronMarkers.slice(-MAX_MARKERS) : electronMarkers
  fs.mkdirSync(path.dirname(MARKERS_FILE), { recursive: true })
  writeJson(MARKERS_FILE, {
    markers: trimmed.map((m) => ({
      game_name: m.game,
      timestamp: m.timestamp,
...
const store = {
  ...
  _games() {
    if (!this._gamesData) this._gamesData = readJson(GAMES_CONFIG_FILE, gamesConfigDefaults)
    return this._gamesData
  },
  _ms() {
    if (!this._msData) this._msData = readJson(MANAGER_SETTINGS_FILE, managerSettingsDefaults)
    return this._msData
  },
  _electron() {
    if (!this._electronData)
      this._electronData = readJson(electronConfigPath, electronConfigDefaults)
    return this._electronData
  },

  _saveGames() {
    writeJson(GAMES_CONFIG_FILE, this._gamesData)
  },
  _saveMs() {
    writeJson(MANAGER_SETTINGS_FILE, this._msData)
  },
  _saveElectron() {
    writeJson(electronConfigPath, this._electronData)
  },
  ...
  get(key) {
    ...
    if (key === 'settings')
      return msToElectronSettings(this._ms(), this._electron().obsRecordingPath, this._electron())

    if (key.startsWith('settings.')) {
      const subKey = key.slice('settings.'.length)
      const settings = msToElectronSettings(
        this._ms(),
        this._electron().obsRecordingPath,
        this._electron()
      )
  ...
  set(key, value) {
    ...
    if (key === 'settings') {
      ...
      if (value.destinationPath) {
        try {
          fs.mkdirSync(normalizePath(value.destinationPath), { recursive: true })
        } catch {}
      }
      return
    }
    if (key.startsWith('settings.')) {
      ...
      if (subKey === 'destinationPath' && value) {
        try {
          fs.mkdirSync(normalizePath(value), { recursive: true })
        } catch {}
      }
      return
    }
