      ...
      try {
        ...
        results.forEach((result, i) => {
          if (result.status === 'rejected') {
            console.warn(
              `[obsWebSocket] Could not duplicate item "${sceneItems[i].sourceName}": ${result.reason?.message}`
            )
          }
        })
  ...
  try {
    return await withOBSConnection(wsSettings, async (obs) => {
      ...
      if (addWindowCapture && windowTitle && windowTitle.trim()) {
        ...
        const windowStr =
          exe && windowClass ? `${trimmedWindow}:${windowClass}:${exe}` : trimmedWindow
    ...
    return await withOBSConnection(wsSettings, async (obs) => {
      ...
      if (fallback) {
        if (currentProgramSceneName === trimmedName) {
          await obs
            .call('SetCurrentProgramScene', { sceneName: fallback.sceneName })
            .catch(() => {})
        }
        if (currentPreviewSceneName === trimmedName) {
          await obs
            .call('SetCurrentPreviewScene', { sceneName: fallback.sceneName })
            .catch(() => {})
        }
      }
      await obs.call('RemoveScene', { sceneName: trimmedName })
