...
function readOBSRecordingPath() {
  ...
  for (const profile of profiles) {
    ...
    try {
      ...
      const recPath = config.SimpleOutput?.FilePath || config.AdvOut?.RecFilePath || null
  ...
  const fallbacks = [path.join(userProfile, 'Videos', 'OBS'), path.join(userProfile, 'Videos')]
