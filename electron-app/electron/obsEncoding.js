...
function isOBSRunning() {
  try {
    return Promise.resolve(getRunningProcessNames().includes('obs64.exe'))
  } catch (err) {
    console.error('[obsEncoding] isOBSRunning:', err.message)
...
function readEncodingSettings(profileDir) {
  ...
  if (settings.output_mode.toLowerCase() === 'advanced') {
    ...
    settings.rec_format =
      simple.RecFormat2 || simple.RecFormat || simple.recformat2 || simple.recformat || 'mp4'
  }
...
function writeEncodingSettings(profileDir, settings) {
  ...
  if (fs.existsSync(encJson)) {
    try {
      enc = JSON.parse(fs.readFileSync(encJson, 'utf-8'))
    } catch {}
  }
