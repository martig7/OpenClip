...
export default function StorageList({
  items, selectedItems, onSelect, lockedRecordings, onLock,
  gameColors, onNavigate, sortBy, sortDir, onColumnSort,
}) {
  ...
  useEffect(() => { scheduleDraw() }, [items, selectedItems, gameColors, sortBy, sortDir, scheduleDraw])
  useEffect(() => {
  ...
  drawRef.current = () => {
    ...
    for (const col of hCols) {
      ...
      if (col.key) {
        ...
        const cx = col.x + 10 + tw + 7, cy = HEADER_H / 2
        ctx.strokeStyle = active ? '#a78bfa' : '#444444'
        ctx.lineWidth = 1.5
        if (!active) { chevron(ctx, cx, cy - 2.5, 5, true); chevron(ctx, cx, cy + 2.5, 5, false) }
    ...
    for (let i = r0; i < r1; i++) {
      ...
      if (isSel) {
        ...
        ctx.beginPath(); ctx.arc(L.xColor + 14, y + ROW_H / 2, 6, 0, Math.PI * 2); ctx.fill()
        ...
        ctx.fillStyle = color
        ctx.beginPath(); ctx.arc(L.xColor + 14, y + ROW_H / 2, 3.5, 0, Math.PI * 2); ctx.fill()
      }
