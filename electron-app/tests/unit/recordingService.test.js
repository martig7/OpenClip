...
describe('createClip', () => {
  ...
  it('maps single audio track correctly', async () => {
    ...
    const calls = cp.execFile.mock.calls.map((c) => c[1])
    const audioCall = calls.find((args) => args.includes('-filter_complex') && args.includes('-map'))
    expect(audioCall).toBeTruthy()
    const joined = audioCall.join(' ')
    expect(joined).toContain('0:a:1')
    expect(joined).toContain('[a0]')
  })
  ...
  it('includes amix for multiple audio tracks', async () => {
    ...
    const calls = cp.execFile.mock.calls.map((c) => c[1])
    const audioCall = calls.find((args) => args.includes('-filter_complex') && args.includes('[mixed]'))
    expect(audioCall).toBeTruthy()
    const joined = audioCall.join(' ')
    expect(joined).toContain('amix')
    expect(joined).toContain('0:a:0')
    expect(joined).toContain('0:a:1')
    expect(joined).toContain('[mixed]')
  })
})
