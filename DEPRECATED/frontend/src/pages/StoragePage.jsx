...
function StoragePage() {
  ...
  const toggleLock = useCallback(async (path) => {
    ...
    try {
      const response = await fetch('/api/storage/lock', {
        method: 'POST',
  ...
  const handleBatchDelete = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/storage/delete-batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const fetchReencodeTracks = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/video/tracks?path=${encodeURIComponent(paths[0])}`)
      const data = await response.json()
  ...
  const handleReencode = useCallback(async () => {
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      try {
        const response = await fetch('/api/reencode', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            source_path: path,
