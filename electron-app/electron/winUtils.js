...
const Process32FirstW = kernel32.func('bool __stdcall Process32FirstW(void *hSnapshot, void *lppe)')
const Process32NextW = kernel32.func('bool __stdcall Process32NextW(void *hSnapshot, void *lppe)')
...
// ── user32.dll ────────────────────────────────────────────────────────────────
const EnumWindows_WU = user32.func('bool __stdcall EnumWindows(WNDPROC_WU *cb, int64 lParam)')
...
function _findExePathForProcess(processName) {
  ...
  if (Process32FirstW(snapshot, procBuf)) {
    do {
      const exeName = procBuf
        .toString('utf16le', EXE_OFFSET, EXE_OFFSET + MAX_PATH * 2)
        .split('\0')[0]
...
function listAudioDevices() {
  ...
  for (let i = 0; i < inCount; i++) {
    ...
    const name = capsBuf
      .toString('utf16le', WAVE_PNAME_OFF, WAVE_PNAME_OFF + MAXPNAMELEN * 2)
      .split('\0')[0]
...
async function extractProcessIcon(processName, outPath) {
  ...
  try {
    ...
    const sres = SHGetFileInfoW(
      exePath,
      0,
      shfi,
      koffi.sizeof(SHFILEINFOW_S),
    ...
    while (Date.now() < deadline) {
      ...
      if (fs.existsSync(resultFile)) {
        try {
          ...
          try {
            fs.rmSync(paramFile, { force: true })
          } catch {}
        }
      }
    }
