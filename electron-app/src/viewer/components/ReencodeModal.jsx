...
export default function ReencodeModal({
  ...
  return (
    <div
      ...
      onMouseDown={(e) => {
        if (e.target === e.currentTarget && !isReencoding) onClose()
      }}
    >
      <div className="modal-content reencode-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="reencode-settings">
            ...
            <>
              <div className="setting-row">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setReencodeSettings({ ...reencodeSettings, crf: parseInt(e.target.value) })
                  }
                  disabled={isReencoding}
                />
                <span>
                  {reencodeSettings.crf}{' '}
                  {reencodeSettings.crf < 20
                    ? '(high)'
                    : reencodeSettings.crf < 24
                      ? '(medium)'
                      : '(low)'}
                </span>
              </div>
              <div className="setting-row">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setReencodeSettings({ ...reencodeSettings, preset: e.target.value })
                  }
                  disabled={isReencoding}
                >
                  ...
                  {reencodeAudioTracks.map((track, i) => (
                    <label key={i} className="track-item">
                      ...
                      <span className="track-detail">
                        {track.codec_name} · {track.channels}ch
                      </span>
                    </label>
                  ))}
                </div>
          ...
          <label className="checkbox-label">
            <input
              ...
              onChange={(e) =>
                setReencodeSettings({ ...reencodeSettings, replaceOriginal: e.target.checked })
              }
              disabled={isReencoding}
            />
        ...
        {isReencoding && (
          <div className="reencode-progress">
            <div className="progress-info">
              <span>
                Reencoding {reencodeProgress.current} of {reencodeProgress.total}
              </span>
            ...
            <div className="progress-bar-container">
              <div
                ...
                style={{
                  width: `${reencodeProgress.total > 0 ? (reencodeProgress.current / reencodeProgress.total) * 100 : 0}%`,
                }}
              />
            </div>
          </div>
        ...
        <div className="modal-actions">
          <button className="btn btn-secondary" onClick={onClose} disabled={isReencoding}>
            Cancel
          </button>
          <button className="btn btn-primary" onClick={onReencode} disabled={isReencoding}>
            {isReencoding ? (
              ...
              <>
                <Package size={14} /> Re-export
              </>
            ) : (
              <>
                <Film size={14} /> Start Reencoding
              </>
            )}
          </button>
        </div>
